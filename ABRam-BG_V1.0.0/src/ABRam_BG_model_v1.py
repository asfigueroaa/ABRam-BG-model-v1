# Agent-based Ramsey growth model with Brown and Green capitals (ABRam-BG) in plain python
#
# Date: September 2025
# Model author: Aida Sarai Figueroa-Alvarez
...
# Information about the ABRam-BG_Model notebook:
# In this notebook we implement the ABRram-BG Model using the Agentpy framework
# (see: https://agentpy.readthedocs.io/en/latest/index.html)

# We simulate utility maximizing agents with binary beliefs that indicate whether they care or not about 
# the environment. 
# 
# For details see the model's ODD and the article:
# - 'Exploring heterogeneity and expectations in technical progress – an exercise in agentization'
# (visit: https://zenodo.org/records/10654996)

# If you are interested in running simulations using the ABRam-BG Model, 
# follow the instructions in the run_the_model_notebook.

#--------------------------------------------------------------------------------------------

#Install Agentpy framework if it is not installed on your computer
#import subprocess, sys
#subprocess.check_call([sys.executable, "-m", "pip", "install", "agentpy"])
...
###################################################
    ...
    def setup(self, alpha, delta, rho, beta, gamma, B0, G0):
        ...
        if self.B == 0 or self.G == 0:
            if self.B == 0: 
                self.I_G = (Farm.alpha*Farm.rho*self.P - (1-delta)*self.K)/(1 + (Farm.alpha*Farm.rho))
                self.I_B = 0
            else:
                self.I_G = 0
                self.I_B = (Farm.alpha*Farm.rho*self.P - (1-delta)*self.K)/(1 + (Farm.alpha*Farm.rho))
...
class Statistician(ap.Agent):
    ...
    def setup(self):
        ...
        #Select brown farms
        brown_farms =  self.farms.select(self.farms.G == 0)
